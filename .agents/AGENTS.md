...
- **编辑原则：精确最小补丁**：修改已有源码时，必须做精确、最小范围 of 补丁式编辑，绝对禁止在没有用户明确要求的情况下整体重写已有文件。
- **Markdown 路径放行白名单**：除了项目运行必需的临时文件外，允许且仅允许新建或修改 `.agents/**`、`docs/superpowers/**`、`**/task.md`、`**/implementation_plan.md`、`**/walkthrough.md` 等开发计划与日志文件。
